...
def get_m1emp_model(df): 
    ...
    subqwifull["sqrt_Emp"] = subqwifull["Emp"] ** 0.5
    ...
    if employmentConfig.TESTFORMULAS == False:
    ...
    if employmentConfig.TESTFORMULAS:
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
        
        # 1. Fitted vs Residuals (Left panel)
        ...
        # 2. QQ Plot (Right panel) - Using R's qqnorm style
        (osm, osr), (slope, intercept, r) = probplot(model.resid, plot=None, fit=True)
        ax2.scatter(osm, osr, alpha=0.6)
        ax2.plot(osm, slope*osm + intercept, 'r--')
        ax2.set_xlabel('Theoretical Quantiles')
        ax2.set_ylabel('Standardized Residuals')
        ax2.set_title('Normal Q-Q')
        textstr = '\n'.join((
            f'{formula}',
            f'Studentized Residual threshold: {employmentConfig.OUTLIER_THRESH}',

            f'Residual SE: {np.sqrt(model.scale):.2f}',
            f'R-squared: {model.rsquared:.3f}',
            f'N: {model.nobs}',
            f'Cook\'s distance influential points detected: {len(influential_indices)}',
            f'Studentized residual outliers detected: {len(outliers)} | {np.round((len(outliers)/len(subqwifull)),3) * 100}%'
            ))
        fig.text(0.15, 0.15, textstr, bbox=dict(facecolor='white', alpha=0.5))
...
def get_m1emp(df, m1empmodel, rseed=5, include_indicator=False):
    '''
    What is the point?
...
def get_m2emp(m1emp, m3emp, stabval, stabF, noisecoef, rseed=5):
    '''
    What is the point?
        get_m1emp() is used as a helper function in get_employmentCounts4().
...
def get_employmentCounts4(df4,m1emp_model, m2emp_noisecoef, rseed=5, include_m1emp_indicator=False):
    '''
    What is the point?
