...
def get_wage_model(df, emp_mat_adj): 
    ...
    wagedf4["sqrt_qp1"]=wagedf4["qp1"].astype(float) ** 0.5
    ...
    outliers = [i for i, r in enumerate(student_resid) if np.abs(r) > wageConfig.OUTLIER_THRESH]
    influential_indices = [i for i, d in enumerate(cooks_d) if d > 1]
    if wageConfig.TESTFORMULAS == False:
    ...
    if wageConfig.TESTFORMULAS:
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize = (12,5))
        #1. Fitted vs Residual
        ...
        textstr = '\n'.join((
            f'{formula}',
            f'Studentized Residual threshold: {wageConfig.OUTLIER_THRESH}',
            ...
            f'Studentized residual outliers detected: {len(outliers)} | {np.round((len(outliers)/len(wagedf4)),3) * 100}%'
            ))
